...
function App(){
 ...
 const [sources,setSources]=useState({}); const [link,setLink]=useState(''); const [sheetNames,setSheetNames]=useState([]); const [selectedSheet,setSelectedSheet]=useState(''); const [linkedMatrices,setLinkedMatrices]=useState({}); const [manualRows,setManualRows]=useState([{roll:'',name:'',college:'',mark:'',added:false}]); const [outputFields,setOutputFields]=useState(()=>OUTPUT_FIELDS.map(([id])=>id)); const [busy,setBusy]=useState(''); const [tab,setTab]=useState('results'); const [showIssueDetails,setShowIssueDetails]=useState(false); const [issueFilter,setIssueFilter]=useState(''); const [issueSearch,setIssueSearch]=useState(''); const [error,setError]=useState('');
 ...
 function persistLibrary(list){setSavedSheets(list);localStorage.setItem('resultflow.savedSheets',JSON.stringify(list))}
 ...
 async function loadGoogleSheet(url,{save=false,preferredTab='',knownTitle=''}={}){try{setBusy('web');setError('');googleWorkbookUrl(url);const res=await fetch(googleWorkbookUrl(url));if(!res.ok)throw new Error('Could not read this sheet. Allow link access, then try again.');const book=XLSX.read(await res.arrayBuffer(),{type:'array',raw:false});const matrices=Object.fromEntries(book.SheetNames.map(name=>[name,XLSX.utils.sheet_to_json(book.Sheets[name],{header:1,defval:'',raw:false})]));const title=knownTitle||workbookTitle(res,book,url);const target=(preferredTab&&matrices[preferredTab]?preferredTab:defaultWebTab(book,matrices));setLinkedMatrices(matrices);setSheetNames(book.SheetNames);activateSheet(target,matrices,url,title);if(save){const id=googleSheetId(url);const next=[...savedSheets.filter(x=>x.id!==id),{id,url,title,webTab:target}];persistLibrary(next);setActiveSavedId(id);setLink('')}}catch(e){setError(e.message)}finally{setBusy('')}}
 ...
 function forgetLink(){const next=savedSheets.filter(x=>x.id!==activeSavedId);persistLibrary(next);setActiveSavedId(next[0]?.id||'');setSheetNames([]);setSelectedSheet('');setLinkedMatrices({});if(sources.web?.linked)setSources(s=>{const {web,...rest}=s;return rest})}
 function chooseTab(name){activateSheet(name);if(activeSavedId){const next=savedSheets.map(x=>x.id===activeSavedId?{...x,webTab:name}:x);persistLibrary(next)}}
 ...
 useEffect(()=>{const first=initialLibrary[0];if(first)loadGoogleSheet(first.url,{preferredTab:first.webTab,knownTitle:first.title})},[]);
 ...
 return <><header><div className="brand"><span>RF</span><div><b>ResultFlow</b><small>Exam processing workspace</small></div></div><div className="security"><ShieldCheck size={17}/> Original files are never edited</div></header>
 <main><section className="hero"><div><span className="eyebrow">NEW PROCESSING SESSION</span><h1>Turn raw exam sheets into a<br/><i>clean, ranked result.</i></h1><p>Upload each source, verify the detected columns, inspect mismatches, then export. Rolls are normalized automatically whenever matching needs it.</p></div><div className="pipeline"><b>Processing pipeline</b>{['Normalize rolls','Exact-match web data','Merge MCQ + CQ','Dense rank'].map((x,i)=><span key={x}><em>{i+1}</em>{x}{i<3&&<ArrowRight size={14}/>}</span>)}</div></section>
 ...
 <div className="or"><span>SAVED GOOGLE SHEET DATABASES</span></div>
